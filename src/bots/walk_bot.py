...
from typing import Dict, List, Tuple, Deque, Optional, Iterable

import random

from config import Graph, Pos
...
class WalkBot(QuoridorBot):
    ...
    def bfs_shortest_path_to_goal(graph: Graph, start: Pos, goal_y: int) -> List[Pos]:
        ...
        raise AssertionError("Unreachable goal: game logic/state is inconsistent.")
    ...
    def select_move(self, state: GraphState, legal_actions: List[Action]) -> Action:
        ...
        blocked_moves = [
            move for move in legal_actions if isinstance(move, BlockedPlayerAction)
        ]
        if blocked_moves and random.random() > 0.5:
            return random.choice(blocked_moves)

        # Otherwise consider normal walking moves
        walk_moves: List[PlayerAction] = [
            move for move in legal_actions if isinstance(move, PlayerAction)
        ]

        if not walk_moves:
            print("no walking moves")
        ...
        for move in walk_moves:
            if getattr(move, "dx", None) == dx and getattr(move, "dy", None) == dy:
                print("match move returned")
                return move

        # Fallback: something desynced; choose any walking move
        print("random move no matching direction found?")
