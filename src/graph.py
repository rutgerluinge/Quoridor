...
from config import Config, Graph, Pos
...
class GraphState:
    ...
    placed_walls: set[tuple[Pos]] = field(init=False)
    ...
    def reset(self):
        """Method to re-initialise the GraphState"""
        self.__post_init__()
    ...
    @staticmethod
    def remove_connection(graph: Graph, pos1: Pos, pos2: Pos):
        """Method to remove a graph connection"""
        if con_list := graph.get(pos1):
            if pos2 in con_list:
                con_list.remove(pos2)

        if con_list := graph.get(pos2):
            if pos1 in con_list:
    ...
    def wall_blocks_players(self, new_wall: WallAction) -> bool:
        ...
        graph_copy = {node: set(neighs) for node, neighs in self.graph.items()}

        # Each wall removes two adjacencies; remove both directions for each
    ...
    def draw_board(self):
        ...
        def _print_cell(r_idx, c_idx):
            if (r_idx, c_idx) == self.player_pos[0]:
                print(Fore.GREEN + "X", end="")
            elif (r_idx, c_idx) == self.player_pos[1]:
